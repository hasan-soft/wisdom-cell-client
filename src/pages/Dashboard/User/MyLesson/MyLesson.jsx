...
const MyLesson = () => {
  ...
  const handleTogglePrivacy = (id, currentPrivacy) => {
    ...
    axiosSecure
      .patch(`/lesson/${id}/privacy`, { privacy: newPrivacy })
      .then(() => {
        refetch();
        Swal.fire({
          icon: "success",
  ...
  const handleToggleAccess = (id, currentAccess) => {
    ...
    axiosSecure
      .patch(`/lesson/${id}/access`, { accessLevel: newAccess })
      .then(() => {
        refetch();
        Swal.fire({
          icon: "success",
  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`${import.meta.env.VITE_API_URL}/my-lesson/${id}`)
          .then((res) => {
  ...
  return (
    <div className="p-4 md:p-10">
      ...
      {/* Desktop Table */}
      <div className="hidden md:block p-4 bg-base-200 rounded-xl shadow">
                ...
                <tr key={lesson._id}>
                  ...
                  <td>
                    <button
                      ...
                      onClick={() =>
                        handleTogglePrivacy(lesson._id, lesson.privacy)
                      }
                    >
                      {lesson.privacy}
                    </button>
                  ...
                  <td className="flex flex-col gap-1">
                    ...
                    {!userData?.isPremium && (
                      <span className="text-xs text-warning">Premium only</span>
                    )}
                  </td>
      ...
      <div className="block md:hidden space-y-4">
        {lessons.map((lesson) => (
          <div
            key={lesson._id}
            className="p-4 bg-base-200 rounded-xl shadow flex flex-col gap-2"
          >
